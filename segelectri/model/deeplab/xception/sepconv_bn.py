...
class SepconvBn(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        if not self.depth_activation:
            inputs = tf.cast(self.activate(inputs, training=training), inputs.dtype) 
          
        result = tf.cast(self.expand_conv(inputs, training=training), inputs.dtype)
        result = tf.cast(self.compress_conv(result, training=training), inputs.dtype)
