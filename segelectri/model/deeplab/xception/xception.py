...
class Xception(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        # entry flow
        result = tf.cast(self.entry_layer1(result, training=training), inputs.dtype)
        result,skip1 = self.entry_layer2(result, training=training)
        result = tf.cast(self.entry_layer3(result, training=training), inputs.dtype)
        ...
        # exit flow
        result = tf.cast(self.exit_layer1(result, training=training), inputs.dtype)
        result = tf.cast(self.exit_layer2(result, training=training), inputs.dtype)
